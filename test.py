...
class StenographerApp:
    ...
    def setup_ui(self):
        ...
        self.create_button(nav_frame, "open_file", self.open_file)
        self.create_button(nav_frame, "save_file", self.save_file)
        self.create_button(nav_frame, "font_size", self.change_font_size)
        self.create_button(nav_frame, "change_language", self.change_language)
        ...
        self.label = customtkinter.CTkLabel(master=input_frame, text=self.get_translation("labels")[self.current_label_index], width=10, font=self.arial_font)
        self.label.pack(side=tk.LEFT, padx=5)

        self.entry = customtkinter.CTkEntry(master=input_frame, placeholder_text=self.get_translation("entry_placeholder"), font=self.arial_font)
    ...
    def create_button(self, frame, key, command):
        button = customtkinter.CTkButton(master=frame, text=self.get_translation(key), command=command, font=self.arial_font, fg_color="#55927f", text_color="black")
    ...
    def get_translation(self, key):
        return TRANSLATIONS[LANGUAGES[self.current_language_index]][key]

    def update_translations(self):
        self.open_button.configure(text=self.get_translation("open_file"))
        self.save_button.configure(text=self.get_translation("save_file"))
        self.change_font_button.configure(text=self.get_translation("font_size"))
        self.change_language_button.configure(text=self.get_translation("change_language"))
        self.label.configure(text=self.get_translation("labels")[self.current_label_index])
        self.entry.configure(placeholder_text=self.get_translation("entry_placeholder"))
    ...
    def save_text(self, event=None):
        ...
        with open(self.file_path, 'a', encoding='utf-8') as f:
            f.write(f"{formatted_now} - {self.get_translation('labels')[self.current_label_index]} - {user_input}\n")
                ...
                with open(self.file_path, 'r', encoding=encoding) as f:
                    ...
                    for line in lines:
                        ...
                        if len(parts) == 3:
                            ...
                            self.text_box.insert(tk.END, f"{label} - ", "question" if label == self.get_translation("labels")[0] else "answer")
                            self.text_box.insert(tk.END, f"{text}\n", "question" if label == self.get_translation("labels")[0] else "answer")
                        else:
    ...
    def change_label(self, event):
        if event.keysym == 'Up':
            self.current_label_index = (self.current_label_index - 1) % len(self.get_translation("labels"))
        elif event.keysym == 'Down':
            self.current_label_index = (self.current_label_index + 1) % len(self.get_translation("labels"))

        current_label = self.get_translation("labels")[self.current_label_index]
        label_color = "#dc6250" if current_label == self.get_translation("labels")[0] else "#2152a5"
